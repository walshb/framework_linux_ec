...
static umode_t fwk_ec_ctrl_visible(struct kobject *kobj,
...
static const struct attribute_group fwk_ec_attr_group = {
	.attrs = __ec_attrs,
	.is_visible = fwk_ec_ctrl_visible,
};

static int fwk_ec_sysfs_probe(struct platform_device *pd)
{
	struct fwk_ec_dev *ec_dev = dev_get_drvdata(pd->dev.parent);
	struct device *dev = &pd->dev;
	int ret;

	ret = sysfs_create_group(&ec_dev->class_dev.kobj, &fwk_ec_attr_group);
	if (ret < 0)
...
static int fwk_ec_sysfs_remove(struct platform_device *pd)
{
	struct fwk_ec_dev *ec_dev = dev_get_drvdata(pd->dev.parent);

	sysfs_remove_group(&ec_dev->class_dev.kobj, &fwk_ec_attr_group);

	return 0;
}

static struct platform_driver fwk_ec_sysfs_driver = {
	.driver = {
		.name = DRV_NAME,
	},
	.probe = fwk_ec_sysfs_probe,
	.remove = fwk_ec_sysfs_remove,
};

module_platform_driver(fwk_ec_sysfs_driver);
