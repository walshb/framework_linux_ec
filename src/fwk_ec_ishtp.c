 ...
 * buffer for them.
 */
#define FWK_ISH_CL_TX_RING_SIZE		8
#define FWK_ISH_CL_RX_RING_SIZE		8

/* ISH CrOS EC Host Commands */
enum fwk_ec_ish_channel {
	FWK_EC_COMMAND = 1,			/* AP->ISH message */
	FWK_MKBP_EVENT = 2,			/* ISH->AP events */
};
...
static const struct ishtp_device_id fwk_ec_ishtp_id_table[] = {
	{ .guid = GUID_INIT(0x7b7154d0, 0x56f4, 0x4bdc,
		  0xb0, 0xd8, 0x9e, 0x7c, 0xda,	0xe0, 0xd6, 0xa0), },
	{ }
};
MODULE_DEVICE_TABLE(ishtp, fwk_ec_ishtp_id_table);
...
struct fwk_ish_out_msg {
	struct header hdr;
	struct ec_host_request ec_request;
} __packed;

struct fwk_ish_in_msg {
	struct header hdr;
	struct ec_host_response ec_response;
} __packed;

#define IN_MSG_EC_RESPONSE_PREAMBLE					\
	offsetof(struct fwk_ish_in_msg, ec_response)

#define OUT_MSG_EC_REQUEST_PREAMBLE					\
	offsetof(struct fwk_ish_out_msg, ec_request)
 ...
 *
 * @fwk_ish_cl: ISHTP firmware client instance.
...
static void ish_evt_handler(struct work_struct *work)
{
	...
	fwk_ec_irq_thread(0, client_data->ec_dev);
}
		    ...
		    u8 *in_msg, size_t in_size)
{
	...
	struct ishtp_cl *fwk_ish_cl = client_data->fwk_ish_cl;
	...
	rv = ishtp_cl_send(fwk_ish_cl, out_msg, out_size);
	if (rv) {
 ...
 * @fwk_ish_cl: Client instance to get stats
 ...
 */
static void process_recv(struct ishtp_cl *fwk_ish_cl,
			 struct ishtp_cl_rb *rb_in_proc, ktime_t timestamp)
{
	size_t data_len = rb_in_proc->buf_idx;
	struct ishtp_cl_data *client_data =
		ishtp_get_client_data(fwk_ish_cl);
	struct device *dev = cl_data_to_dev(client_data);
	struct fwk_ish_in_msg *in_msg =
		(struct fwk_ish_in_msg *)rb_in_proc->buffer.data;
...
static void ish_event_cb(struct ishtp_cl_device *cl_device)
{
	...
	struct ishtp_cl	*fwk_ish_cl = ishtp_get_drvdata(cl_device);
	ktime_t timestamp;
	 ...
	 * timestamps.
	 */
	timestamp = fwk_ec_get_time_ns();

	while ((rb_in_proc = ishtp_cl_rx_get_rb(fwk_ish_cl)) != NULL) {
		/* Decide what to do with received data */
		process_recv(fwk_ish_cl, rb_in_proc, timestamp);
	}
}

/**
 * fwk_ish_init() - Init function for ISHTP client
 * @fwk_ish_cl: ISHTP client instance
 *
 * This function complete the initializtion of the client.
 *
 * Return: 0 for success, negative error code for failure.
 */
static int fwk_ish_init(struct ishtp_cl *fwk_ish_cl)
{
	int rv;
	struct ishtp_device *dev;
	struct ishtp_fw_client *fw_client;
	struct ishtp_cl_data *client_data = ishtp_get_client_data(fwk_ish_cl);

	rv = ishtp_cl_link(fwk_ish_cl);
	if (rv) {
		dev_err(cl_data_to_dev(client_data),
	...
	dev = ishtp_get_ishtp_device(fwk_ish_cl);

	/* Connect to firmware client */
	ishtp_set_tx_ring_size(fwk_ish_cl, FWK_ISH_CL_TX_RING_SIZE);
	ishtp_set_rx_ring_size(fwk_ish_cl, FWK_ISH_CL_RX_RING_SIZE);

	fw_client = ishtp_fw_cl_get_client(dev, &fwk_ec_ishtp_id_table[0].guid);
	if (!fw_client) {
		dev_err(cl_data_to_dev(client_data),
	...
	ishtp_cl_set_fw_client_id(fwk_ish_cl,
				  ishtp_get_fw_client_id(fw_client));
	ishtp_set_connection_state(fwk_ish_cl, ISHTP_CL_CONNECTING);

	rv = ishtp_cl_connect(fwk_ish_cl);
	if (rv) {
		dev_err(cl_data_to_dev(client_data),
			"client connect fail\n");
		goto err_cl_unlink;
...
err_cl_unlink:
	ishtp_cl_unlink(fwk_ish_cl);
	return rv;
}

/**
 * fwk_ish_deinit() - Deinit function for ISHTP client
 * @fwk_ish_cl: ISHTP client instance
 *
 * Unlink and free fwk_ec client
 */
static void fwk_ish_deinit(struct ishtp_cl *fwk_ish_cl)
{
	ishtp_set_connection_state(fwk_ish_cl, ISHTP_CL_DISCONNECTING);
	ishtp_cl_disconnect(fwk_ish_cl);
	ishtp_cl_unlink(fwk_ish_cl);
	ishtp_cl_flush_queues(fwk_ish_cl);

	/* Disband and free all Tx and Rx client-level rings */
	ishtp_cl_free(fwk_ish_cl);
}

/**
 * prepare_fwk_ec_rx() - Check & prepare receive buffer
 * @ec_dev: CrOS EC MFD device.
 * @in_msg: Incoming message buffer
 * @msg: fwk_ec command used to send & receive data
 *
 * Return: 0 for success, negative error code for failure.
 *
 * Check the received buffer. Convert to fwk_ec_command format.
 */
static int prepare_fwk_ec_rx(struct fwk_ec_device *ec_dev,
			      const struct fwk_ish_in_msg *in_msg,
			      struct fwk_ec_command *msg)
{
	...
	rv = fwk_ec_check_result(ec_dev, msg);
	if (rv < 0)
		return rv;
	...
	offset = sizeof(struct fwk_ish_in_msg);
...
static int fwk_ec_pkt_xfer_ish(struct fwk_ec_device *ec_dev,
				struct fwk_ec_command *msg)
{
	int rv;
	struct ishtp_cl *fwk_ish_cl = ec_dev->priv;
	struct ishtp_cl_data *client_data = ishtp_get_client_data(fwk_ish_cl);
	struct device *dev = cl_data_to_dev(client_data);
	struct fwk_ish_in_msg *in_msg = (struct fwk_ish_in_msg *)ec_dev->din;
	struct fwk_ish_out_msg *out_msg =
		(struct fwk_ish_out_msg *)ec_dev->dout;
	size_t in_size = sizeof(struct fwk_ish_in_msg) + msg->insize;
	size_t out_size = sizeof(struct fwk_ish_out_msg) + msg->outsize;
	...
	out_msg->hdr.channel = FWK_EC_COMMAND;
	out_msg->hdr.status = 0;

	ec_dev->dout += OUT_MSG_EC_REQUEST_PREAMBLE;
	rv = fwk_ec_prepare_tx(ec_dev, msg);
	if (rv < 0)
		goto end_error;
	ec_dev->dout -= OUT_MSG_EC_REQUEST_PREAMBLE;
	...
	rv = prepare_fwk_ec_rx(ec_dev, in_msg, msg);
	if (rv)
		goto end_error;
...
static int fwk_ec_dev_init(struct ishtp_cl_data *client_data)
{
	struct fwk_ec_device *ec_dev;
	struct device *dev = cl_data_to_dev(client_data);
	...
	ec_dev->dev = dev;
	ec_dev->priv = client_data->fwk_ish_cl;
	ec_dev->cmd_xfer = NULL;
	ec_dev->pkt_xfer = fwk_ec_pkt_xfer_ish;
	ec_dev->phys_name = dev_name(dev);
	ec_dev->din_size = sizeof(struct fwk_ish_in_msg) +
			   sizeof(struct ec_response_get_protocol_info);
	ec_dev->dout_size = sizeof(struct fwk_ish_out_msg);

	return fwk_ec_register(ec_dev);
}
...
static void reset_handler(struct work_struct *work)
{
	...
	fwk_ish_cl = client_data->fwk_ish_cl;
	...
	fwk_ish_cl = ishtp_cl_allocate(cl_device);
	if (!fwk_ish_cl) {
		up_write(&init_lock);
		return;
	}

	ishtp_set_drvdata(cl_device, fwk_ish_cl);
	ishtp_set_client_data(fwk_ish_cl, client_data);
	client_data->fwk_ish_cl = fwk_ish_cl;

	rv = fwk_ish_init(fwk_ish_cl);
	if (rv) {
		ishtp_cl_free(fwk_ish_cl);
	...
	client_data->ec_dev->priv = client_data->fwk_ish_cl;
...
/**
 * fwk_ec_ishtp_probe() - ISHTP client driver probe callback
 * @cl_device: ISHTP client device instance
 *
 * Return: 0 for success, negative error code for failure.
 */
static int fwk_ec_ishtp_probe(struct ishtp_cl_device *cl_device)
{
	int rv;
	struct ishtp_cl *fwk_ish_cl;
	struct ishtp_cl_data *client_data =
	...
	rv = fwk_ec_dev_init(client_data);
	if (rv) {
		down_write(&init_lock);
		goto end_fwk_ec_dev_init_error;
	}

	return 0;

end_fwk_ec_dev_init_error:
	ishtp_set_connection_state(fwk_ish_cl, ISHTP_CL_DISCONNECTING);
	ishtp_cl_disconnect(fwk_ish_cl);
	ishtp_cl_unlink(fwk_ish_cl);
	ishtp_cl_flush_queues(fwk_ish_cl);
...
/**
 * fwk_ec_ishtp_remove() - ISHTP client driver remove callback
 * @cl_device: ISHTP client device instance
 *
 * Return: 0
 */
static void fwk_ec_ishtp_remove(struct ishtp_cl_device *cl_device)
{
	struct ishtp_cl	*fwk_ish_cl = ishtp_get_drvdata(cl_device);
	struct ishtp_cl_data *client_data = ishtp_get_client_data(fwk_ish_cl);
	...
	fwk_ish_deinit(fwk_ish_cl);
	ishtp_put_device(cl_device);
}

/**
 * fwk_ec_ishtp_reset() - ISHTP client driver reset callback
 * @cl_device: ISHTP client device instance
 *
 * Return: 0
 */
static int fwk_ec_ishtp_reset(struct ishtp_cl_device *cl_device)
{
	struct ishtp_cl	*fwk_ish_cl = ishtp_get_drvdata(cl_device);
	struct ishtp_cl_data *client_data = ishtp_get_client_data(fwk_ish_cl);
...
/**
 * fwk_ec_ishtp_suspend() - ISHTP client driver suspend callback
 * @device: device instance
 *
 * Return: 0 for success, negative error code for failure.
 */
static int __maybe_unused fwk_ec_ishtp_suspend(struct device *device)
{
	struct ishtp_cl_device *cl_device = ishtp_dev_to_cl_device(device);
	struct ishtp_cl	*fwk_ish_cl = ishtp_get_drvdata(cl_device);
	struct ishtp_cl_data *client_data = ishtp_get_client_data(fwk_ish_cl);

	return fwk_ec_suspend(client_data->ec_dev);
}

/**
 * fwk_ec_ishtp_resume() - ISHTP client driver resume callback
 * @device: device instance
 *
 * Return: 0 for success, negative error code for failure.
 */
static int __maybe_unused fwk_ec_ishtp_resume(struct device *device)
{
	struct ishtp_cl_device *cl_device = ishtp_dev_to_cl_device(device);
	struct ishtp_cl	*fwk_ish_cl = ishtp_get_drvdata(cl_device);
	struct ishtp_cl_data *client_data = ishtp_get_client_data(fwk_ish_cl);

	return fwk_ec_resume(client_data->ec_dev);
}

static SIMPLE_DEV_PM_OPS(fwk_ec_ishtp_pm_ops, fwk_ec_ishtp_suspend,
			 fwk_ec_ishtp_resume);

static struct ishtp_cl_driver	fwk_ec_ishtp_driver = {
	.name = "fwk_ec_ishtp",
	.id = fwk_ec_ishtp_id_table,
	.probe = fwk_ec_ishtp_probe,
	.remove = fwk_ec_ishtp_remove,
	.reset = fwk_ec_ishtp_reset,
	.driver = {
		.pm = &fwk_ec_ishtp_pm_ops,
	},
};

static int __init fwk_ec_ishtp_mod_init(void)
{
	return ishtp_cl_driver_register(&fwk_ec_ishtp_driver, THIS_MODULE);
}

static void __exit fwk_ec_ishtp_mod_exit(void)
{
	ishtp_cl_driver_unregister(&fwk_ec_ishtp_driver);
}

module_init(fwk_ec_ishtp_mod_init);
module_exit(fwk_ec_ishtp_mod_exit);
