...
static struct fwk_ec_platform ec_p = {
	.ec_name = FWK_EC_DEV_NAME,
	.cmd_offset = EC_CMD_PASSTHRU_OFFSET(FWK_EC_DEV_EC_INDEX),
};

static struct fwk_ec_platform pd_p = {
	.ec_name = FWK_EC_DEV_PD_NAME,
	.cmd_offset = EC_CMD_PASSTHRU_OFFSET(FWK_EC_DEV_PD_INDEX),
};

/**
 * fwk_ec_irq_handler() - top half part of the interrupt handler
 * @irq: IRQ id
 * @data: (ec_dev) Device with events to process.
 *
 * Return: Wakeup the bottom half
 */
static irqreturn_t fwk_ec_irq_handler(int irq, void *data)
{
...
/**
 * fwk_ec_handle_event() - process and forward pending events on EC
 ...
 * called again.
 */
static bool fwk_ec_handle_event(struct fwk_ec_device *ec_dev)
{
	bool wake_event;
	bool ec_has_more_events;
	int ret;

	ret = fwk_ec_get_next_event(ec_dev, &wake_event, &ec_has_more_events);

	/*
	 * Signal only if wake host events or any interrupt if
	 * fwk_ec_get_next_event() returned an error (default value for
	 * wake_event is true)
	 */
...
/**
 * fwk_ec_irq_thread() - bottom half part of the interrupt handler
 * @irq: IRQ id
 * @data: (ec_dev) Device with events to process.
 *
...
irqreturn_t fwk_ec_irq_thread(int irq, void *data)
{
	...
	do {
		ec_has_more_events = fwk_ec_handle_event(ec_dev);
	} while (ec_has_more_events);

	return IRQ_HANDLED;
}
EXPORT_SYMBOL(fwk_ec_irq_thread);

static int fwk_ec_sleep_event(struct fwk_ec_device *ec_dev, u8 sleep_event)
{
	int ret;
	struct {
	...
	ret = fwk_ec_cmd_xfer_status(ec_dev, &buf.msg);
...
static int fwk_ec_ready_event(struct notifier_block *nb,
			       unsigned long queued_during_suspend,
			       void *_notify)
{
	struct fwk_ec_device *ec_dev = container_of(nb, struct fwk_ec_device,
						     notifier_ready);
...
static int fwk_ec_mutex_lock(struct fwk_ec_device *ec_dev)
{
...
static int fwk_ec_mutex_unlock(struct fwk_ec_device *ec_dev)
{
...
/**
 * fwk_ec_register() - Register a new ChromeOS EC, using the provided info.
...
int fwk_ec_register(struct fwk_ec_device *ec_dev)
{
	...
	if (!ec_dev->ec_mutex_lock) {
		ec_dev->ec_mutex_lock = fwk_ec_mutex_lock;
		ec_dev->ec_mutex_unlock = fwk_ec_mutex_unlock;
	}
	...
	ec_dev->ec = platform_device_register_data(ec_dev->dev, "fwk-ec-dev",
					PLATFORM_DEVID_AUTO, &ec_p,
					sizeof(struct fwk_ec_platform));
	if (IS_ERR(ec_dev->ec)) {
		dev_err(ec_dev->dev,
	 ...
	 */
	err = fwk_ec_sleep_event(ec_dev, 0);
	if (err < 0)
	...
	if (ec_dev->mkbp_event_supported) {
		 ...
		 * event.
		 */
		ec_dev->notifier_ready.notifier_call = fwk_ec_ready_event;
...
EXPORT_SYMBOL(fwk_ec_register);

/**
 * fwk_ec_unregister() - Remove a ChromeOS EC.
...
static void fwk_ec_send_suspend_event(struct fwk_ec_device *ec_dev)
{
	...
	ret = fwk_ec_sleep_event(ec_dev, sleep_event);
	if (ret < 0)
...
static void fwk_ec_report_events_during_suspend(struct fwk_ec_device *ec_dev)
{
	bool wake_event;

	while (ec_dev->mkbp_event_supported &&
	       fwk_ec_get_next_event(ec_dev, &wake_event, NULL) > 0) {
		blocking_notifier_call_chain(&ec_dev->event_notifier,
					     1, ec_dev);
