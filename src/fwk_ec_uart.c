 ...
 *			fwk_ec_uart_pkt_xfer() and fwk_ec_uart_rx_bytes()
 *			callback.
 ...
 *			on failure.
 * @wait_queue:	Wait queue EC response where the fwk_ec sends request
 *			to EC and waits
 */
...
/**
 * struct fwk_ec_uart - information about a uart-connected EC
 *
 ...
 * @response:		Response info passing between fwk_ec_uart_pkt_xfer()
 *			and fwk_ec_uart_rx_bytes()
 */
struct fwk_ec_uart {
...
static int fwk_ec_uart_rx_bytes(struct serdev_device *serdev,
				 const u8 *data,
				 size_t count)
{
	struct ec_host_response *host_response;
	struct fwk_ec_device *ec_dev = serdev_device_get_drvdata(serdev);
	struct fwk_ec_uart *ec_uart = ec_dev->priv;
	 ...
	 * buffer in din by fwk_ec. This will ensure that if EC sends more
...
static int fwk_ec_uart_pkt_xfer(struct fwk_ec_device *ec_dev,
...
static int fwk_ec_uart_resource(struct acpi_resource *ares, void *data)
{
	struct fwk_ec_uart *ec_uart = data;
...
static int fwk_ec_uart_acpi_probe(struct fwk_ec_uart *ec_uart)
{
	int ret;
	LIST_HEAD(resources);
	struct acpi_device *adev = ACPI_COMPANION(&ec_uart->serdev->dev);

	ret = acpi_dev_get_resources(adev, &resources, fwk_ec_uart_resource, ec_uart);
	if (ret < 0)
		return ret;
...
static const struct serdev_device_ops fwk_ec_uart_client_ops = {
	.receive_buf = fwk_ec_uart_rx_bytes,
};

static int fwk_ec_uart_probe(struct serdev_device *serdev)
{
	struct device *dev = &serdev->dev;
	struct fwk_ec_device *ec_dev;
	struct fwk_ec_uart *ec_uart;
	int ret;
	...
	ret = fwk_ec_uart_acpi_probe(ec_uart);
	if (ret < 0) {
	...
	/* Initialize ec_dev for fwk_ec  */
	ec_dev->phys_name = dev_name(dev);
	ec_dev->dev = dev;
	ec_dev->priv = ec_uart;
	ec_dev->irq = ec_uart->irq;
	ec_dev->cmd_xfer = NULL;
	ec_dev->pkt_xfer = fwk_ec_uart_pkt_xfer;
	ec_dev->din_size = sizeof(struct ec_host_response) +
			   sizeof(struct ec_response_get_protocol_info);
	ec_dev->dout_size = sizeof(struct ec_host_request);

	serdev_device_set_client_ops(serdev, &fwk_ec_uart_client_ops);

	return fwk_ec_register(ec_dev);
}

static void fwk_ec_uart_remove(struct serdev_device *serdev)
{
	struct fwk_ec_device *ec_dev = serdev_device_get_drvdata(serdev);
...
static int __maybe_unused fwk_ec_uart_suspend(struct device *dev)
{
...
static int __maybe_unused fwk_ec_uart_resume(struct device *dev)
{
	struct fwk_ec_device *ec_dev = dev_get_drvdata(dev);
...
static SIMPLE_DEV_PM_OPS(fwk_ec_uart_pm_ops, fwk_ec_uart_suspend,
			 fwk_ec_uart_resume);

static const struct of_device_id fwk_ec_uart_of_match[] = {
	{ .compatible = "google,fwk-ec-uart" },
	{}
};
MODULE_DEVICE_TABLE(of, fwk_ec_uart_of_match);

#ifdef CONFIG_ACPI
static const struct acpi_device_id fwk_ec_uart_acpi_id[] = {
	{ "GOOG0019", 0 },
	{}
};

MODULE_DEVICE_TABLE(acpi, fwk_ec_uart_acpi_id);
#endif

static struct serdev_device_driver fwk_ec_uart_driver = {
	.driver	= {
		.name	= "fwk-ec-uart",
		.acpi_match_table = ACPI_PTR(fwk_ec_uart_acpi_id),
		.of_match_table = fwk_ec_uart_of_match,
		.pm	= &fwk_ec_uart_pm_ops,
	},
	.probe		= fwk_ec_uart_probe,
	.remove		= fwk_ec_uart_remove,
};

module_serdev_device_driver(fwk_ec_uart_driver);
