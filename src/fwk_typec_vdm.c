...
void fwk_typec_handle_vdm_attention(struct fwk_typec_data *typec, int port_num)
{
	...
	do {
		ret = fwk_ec_cmd(typec->ec, 0, EC_CMD_TYPEC_VDM_RESPONSE, &req,
		...
		amode = typec_match_altmode(typec->ports[port_num]->port_altmode,
					    FWK_EC_ALTMODE_MAX, svid, PD_VDO_OPOS(hdr));
		if (!amode) {
			dev_err(typec->dev,
...
void fwk_typec_handle_vdm_response(struct fwk_typec_data *typec, int port_num)
{
	...
	amode = typec_match_altmode(typec->ports[port_num]->port_altmode, FWK_EC_ALTMODE_MAX,
...
static int fwk_typec_port_amode_enter(struct typec_altmode *amode, u32 *vdo)
{
	struct fwk_typec_port *port = typec_altmode_get_drvdata(amode);
	...
	return fwk_ec_cmd(port->typec_data->ec, 0, EC_CMD_TYPEC_CONTROL, &req,
			   sizeof(req), NULL, 0);
}

static int fwk_typec_port_amode_vdm(struct typec_altmode *amode, const u32 hdr,
...
struct typec_altmode_ops port_amode_ops = {
	.enter = fwk_typec_port_amode_enter,
	.vdm = fwk_typec_port_amode_vdm,
};
